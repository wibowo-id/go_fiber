...
// function to return pretty json response (encrypts id / *_id UUID fields when key is set)
func Resp(c *fiber.Ctx, resp Response) error {
	...
	// encrypt id / *_id UUID fields in response data and meta
	if resp.Data != nil {
		encData, err := crypto.EncryptIDsInResponse(resp.Data)
		if err == nil {
			resp.Data = encData
		}
	}
	if resp.Meta != nil {
		encMeta, err := crypto.EncryptIDsInResponse(resp.Meta)
		if err == nil {
			resp.Meta = encMeta
		}
	}
