...
import (
	...
	idcrypto "go_fiber_wibowo/utils/crypto"
	...
	lifecycle.Append(
		fx.Hook{
			OnStart: func(ctx context.Context) error {
				// Set ID encryption key for encrypt/decrypt of id and *_id in request/response
				if cfg.App.IDEncryptionKey != "" {
					idcrypto.SetKey(cfg.App.IDEncryptionKey)
				}
				...
				migrate := flag.Bool("migrate", cfg.DB.Migration, "migrate the database")
				seeder := flag.Bool("seed", cfg.DB.Seeder, "seed the database")
				flag.Parse()
