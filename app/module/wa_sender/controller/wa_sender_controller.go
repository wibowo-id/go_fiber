...
type waSenderController struct {
	waSenderService service.WaSenderService
}

type WaSenderController interface {
	SendOtp(c *fiber.Ctx) error
	GenerateQr(c *fiber.Ctx) error
}

func NewWaSenderController(waSenderService service.WaSenderService) WaSenderController {
	return &waSenderController{
		waSenderService: waSenderService,
	}
}

func (_i *waSenderController) SendOtp(c *fiber.Ctx) error {
	req := new(request.OtpRequest)
	if err := response.ParseAndValidate(c, req); err != nil {
		return err
	}

	res, err := _i.waSenderService.SendOtp(*req)
	if err != nil {
		return err
	}
